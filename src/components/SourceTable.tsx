...
export default function SourceTable({ sources }: Props) {
  ...
  const displayed = expanded ? activeSources : activeSources.slice(0, 5);

  return (
    <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-3xl p-6">
      <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
        📡 Sources météo consultées
        <span className="text-sm font-normal text-sky-300">
          ({activeSources.filter((s) => !s.error).length}/{activeSources.length} actives)
        </span>
      </h2>
      ...
      {activeSources.length > 5 && (
        <button
          ...
          className="mt-4 text-sky-400 hover:text-sky-200 text-sm transition-colors"
        >
  ...
  return (
    ...
    >
      ...
      <td className="py-3 px-2 text-center text-white">
        {source.temperature !== null ? `${source.temperature.toFixed(1)}°C` : "—"}
      </td>
      <td className="py-3 px-2 text-center text-white">
        {source.windSpeed !== null ? `${Math.round(source.windSpeed)} km/h` : "—"}
      </td>
      <td className="py-3 px-2 text-center text-white">
        {source.humidity !== null ? `${Math.round(source.humidity)}%` : "—"}
      </td>
      <td className="py-3 px-2 text-center text-white">
        {source.precipitation !== null ? `${source.precipitation.toFixed(1)} mm` : "—"}
      </td>
